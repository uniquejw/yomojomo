function pagelist() {
	...
	fetch(`/report/listselect?no=${pageNo}&cutno=${cutCount}&searchKeyword=${searchKeyword}`)
		.then(function(response) {
			...
			for (i = 0; i<lists.length; i++) {
				if (lists[i].valid === 0) {
					lists[i].valid = "대기중";
				} else if (lists[i].valid === 1) {
					lists[i].valid = "경고";
				} else if (lists[i].valid === 2) {
					lists[i].valid = "처리 완료";
				} else {
					lists[i].valid = "무혐의";
				}
	...
	fetch(`/report/reportcountselect?searchKeyword=${searchKeyword}`)
		.then(function(response) {
			return response.json();
		}).then(function(lists) {
			allBtn(lists);
	...
	$(document).on('click', 'a', function(e) {
		...
		passNo = no;
		location.href = "/jaewon/admin/report/index.html?no=" + passNo + "&cutno=" + cutno + "&searchKeyword=" + searchKeyword;
	})
...
$(document).on('click', '.searchBtn', function() {
	...
	location.href = "/jaewon/admin/report/index.html?no=1" + "&cutno=" + cutno + "&searchKeyword=" + searchKeyword;
})
