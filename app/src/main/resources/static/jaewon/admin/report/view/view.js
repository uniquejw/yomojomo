function list() {
	...
	fetch(`/report/get?no=${no}`).then(function(response) {
		return response.json();
	}).then(function(lists) {
		console.log(lists)
		if (lists.valid === 0) {
			lists.valid = "대기중";
		} else if (lists.valid === 1) {
			lists.valid = "경고";
		} else if (lists.valid === 2) {
			lists.valid = "처리 완료";
		} else {
			lists.valid = "무혐의";
		}
...
$(document).on('click', '.update-btn', function() {
	...
	location.href = "/jaewon/admin/report/view/uindex.html?no=" + xNo;
})
...
$(document).on('click', '.update-finish-btn', function() {
	if (confirm('수정하시겠습니까?')) {
		...
		var xValid = $('input[name="penalty"]:checked').val()
		var now = $.now();

		fd.append("no", xNo);
		fd.append("valid", xValid);
