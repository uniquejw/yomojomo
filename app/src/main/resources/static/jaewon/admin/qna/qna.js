function pagelist() {
	...
	fetch(`/noticeQuestion/listselect?no=${pageNo}&cutno=${cutCount}&searchKeyword=${searchKeyword}`)
	...
	fetch(`/noticeQuestion/nqcountselect?searchKeyword=${searchKeyword}`)
		.then(function(response) {
			return response.json();
		}).then(function(lists) {
			allBtn(lists);
	...
	$(document).on('click', 'a', function(e) {
		...
		passNo = no;
		location.href = "/jaewon/admin/qna/index.html?no=" + passNo + "&cutno=" + cutno + "&searchKeyword=" + searchKeyword;
	})
...
$(document).on('click', '.searchBtn', function() {
	...
	location.href = "/jaewon/admin/qna/index.html?no=1" + "&cutno=" + cutno + "&searchKeyword=" + searchKeyword;
})
