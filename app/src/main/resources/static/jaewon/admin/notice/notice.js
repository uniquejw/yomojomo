function pagelist() {
	...
	fetch(`/notice/listselect?no=${pageNo}&cutno=${cutCount}&searchKeyword=${searchKeyword}`)
	...
	fetch(`/notice/noticecountselect?searchKeyword=${searchKeyword}`)
		.then(function(response) {
			return response.json();
		}).then(function(lists) {
			allBtn(lists);
	...
	$(document).on('click', 'a', function(e) {
		...
		passNo = no;
		location.href = "/jaewon/admin/notice/index.html?no=" + passNo + "&cutno=" + cutno + "&searchKeyword=" + searchKeyword;
	})
...
$(document).on('click', '.searchBtn', function() {
	...
	location.href = "/jaewon/admin/notice/index.html?no=1" + "&cutno=" + cutno + "&searchKeyword=" + searchKeyword;
})


$(document).on('click', '.write', function() {
	location.href="/jaewon/admin/notice/create/index.html"
	})
