function pagelist() {
	...
	fetch(`/group/listselect?no=${pageNo}&cutno=${cutCount}&searchKeyword=${searchKeyword}`)
		.then(function(response) {
			return response.json();
		}).then(function(lists) {
			console.log(lists)
			for (i = 0; i < lists.length; i++) {
				if (lists[i].status === 0 || lists[i].status === null) {
					lists[i].status = "없음";
				} else {
					lists[i].status = "제재 중";
				}
	...
	fetch(`/group/groupcountselect?searchKeyword=${searchKeyword}`)
		.then(function(response) {
			return response.json();
		}).then(function(lists) {
			allBtn(lists);
	...
	$(document).on('click', 'a', function(e) {
		...
		passNo = no;
		location.href = "/jaewon/admin/group/index.html?no=" + passNo + "&cutno=" + cutno + "&searchKeyword=" + searchKeyword;
	})
...
$(document).on('click', '.searchBtn', function() {
	...
	location.href = "/jaewon/admin/group/index.html?no=1" + "&cutno=" + cutno + "&searchKeyword=" + searchKeyword;
})
