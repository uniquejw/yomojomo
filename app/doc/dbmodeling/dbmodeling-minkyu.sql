...
-- 가입신청서기본답변
DROP TABLE IF EXISTS gms_applyform_default RESTRICT;
...
CREATE TABLE gms_memb (
  ...
  de_img        BOOLEAN      NULL     COMMENT '기본사진여부' -- 기본사진여부
)
COMMENT '회원';
...
CREATE TABLE gms_accounting_status (
  ...
  g_no    INTEGER NOT NULL COMMENT '모임번호', -- 모임번호
  memb_no INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
  pay_dt  DATE    NULL     DEFAULT null COMMENT '납부일' -- 납부일
)
COMMENT '모금납부내역';
...
CREATE TABLE gms_destination (
  ...
  location_cate_id VARCHAR(50) NOT NULL COMMENT '장소카테고리', -- 장소카테고리
  place_URL        LONGTEXT    NOT NULL COMMENT '장소URL' -- 장소URL
)
COMMENT '최종모임장소';
...
-- 가입신청서기본답변
CREATE TABLE gms_applyform_default (
  g_no     INTEGER  NOT NULL COMMENT '모임번호', -- 모임번호
  memb_no  INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
...
-- 가입신청서기본답변
ALTER TABLE gms_applyform_default
...
ALTER TABLE gms_select_midpoint
  ADD CONSTRAINT FK_gms_calendar_TO_gms_select_midpoint -- 모임내부 일정 -> 맴버장소입력
    ...
    REFERENCES gms_calendar ( -- 모임내부 일정
      cal_no -- 일정번호
    );
...
-- 가입신청서기본답변
ALTER TABLE gms_applyform_default
  ADD CONSTRAINT FK_gms_memb_TO_gms_applyform_default2 -- 회원 -> 가입신청서기본답변2
    FOREIGN KEY (
      memb_no2 -- 회원번호2
    )
    REFERENCES gms_memb ( -- 회원
