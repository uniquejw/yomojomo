-- 회원
DROP TABLE IF EXISTS `BTSdb`.`gms_memb` RESTRICT;

-- 메인카테고리
DROP TABLE IF EXISTS `BTSdb`.`gms_main_cate` RESTRICT;

-- 최근검색어
DROP TABLE IF EXISTS `BTSdb`.`gms_keyword` RESTRICT;

-- 소모임
DROP TABLE IF EXISTS `BTSdb`.`gms_group` RESTRICT;

-- 활동지역
DROP TABLE IF EXISTS `BTSdb`.`gms_activelocal` RESTRICT;

-- 모임목적
DROP TABLE IF EXISTS `BTSdb`.`gms_purpose` RESTRICT;

-- 모임태그
DROP TABLE IF EXISTS `BTSdb`.`gms_grouptag` RESTRICT;

-- 모임가입신청서
DROP TABLE IF EXISTS `BTSdb`.`gms_applyform` RESTRICT;

-- 모임게시글
DROP TABLE IF EXISTS `BTSdb`.`gms_board` RESTRICT;

-- 모임내부 게시글 댓글
DROP TABLE IF EXISTS `BTSdb`.`gms_comment` RESTRICT;

-- 모임게시글사진
DROP TABLE IF EXISTS `BTSdb`.`gms_photo` RESTRICT;

-- 모임내부 일정
DROP TABLE IF EXISTS `BTSdb`.`gms_calendar` RESTRICT;

-- 모임내부 첨부파일
DROP TABLE IF EXISTS `BTSdb`.`gms_file` RESTRICT;

-- 모임맴버
DROP TABLE IF EXISTS `BTSdb`.`gms_join_memb` RESTRICT;

-- 모금
DROP TABLE IF EXISTS `BTSdb`.`gms_accounting` RESTRICT;

-- 맴버장소입력
DROP TABLE IF EXISTS `BTSdb`.`gms_select_midpoint` RESTRICT;

-- 고객센터 자주하는 질문
DROP TABLE IF EXISTS `BTSdb`.`gms_faq` RESTRICT;

-- 고객센터 공지사항
DROP TABLE IF EXISTS `BTSdb`.`gms_notice` RESTRICT;

-- 고객센터 문의하기
DROP TABLE IF EXISTS `BTSdb`.`gms_notice_question` RESTRICT;

-- 관심활동지역
DROP TABLE IF EXISTS `BTSdb`.`gms_final_activelocal` RESTRICT;

-- 관심모임목적
DROP TABLE IF EXISTS `BTSdb`.`gms_final_purpose` RESTRICT;

-- 모금유형
DROP TABLE IF EXISTS `BTSdb`.`gms_accounting_cate` RESTRICT;

-- 모금납부내역
DROP TABLE IF EXISTS `BTSdb`.`gms_accounting_status` RESTRICT;

-- 모임게시글스크랩
DROP TABLE IF EXISTS `BTSdb`.`gms_scrap` RESTRICT;

-- 질문유형
DROP TABLE IF EXISTS `BTSdb`.`gms_query_cate` RESTRICT;

-- SNS계정
DROP TABLE IF EXISTS `BTSdb`.`gms_final_SNS` RESTRICT;

-- SNS
DROP TABLE IF EXISTS `BTSdb`.`gms_SNS` RESTRICT;

-- 최종모임장소
DROP TABLE IF EXISTS `BTSdb`.`gms_destination` RESTRICT;

-- 모임등급유형
DROP TABLE IF EXISTS `BTSdb`.`gms_memb_grade` RESTRICT;

-- pickme게시판
DROP TABLE IF EXISTS `BTSdb`.`gms_pickme` RESTRICT;

-- 신고
DROP TABLE IF EXISTS `BTSdb`.`gms_report` RESTRICT;

-- 신고유형
DROP TABLE IF EXISTS `BTSdb`.`gms_report_cate` RESTRICT;

-- 초대장저장공간
DROP TABLE IF EXISTS `BTSdb`.`gms_invite_box` RESTRICT;

-- 모임가입신청서질문목록
DROP TABLE IF EXISTS `BTSdb`.`gms_applyform_question` RESTRICT;

-- 모임가입신청서질문목록답변
DROP TABLE IF EXISTS `BTSdb`.`gms_applyform_answer` RESTRICT;

-- 가입신청서기본답변
DROP TABLE IF EXISTS `BTSdb`.`gms_applyform_default` RESTRICT;

-- 회원
CREATE TABLE `BTSdb`.`gms_memb` (
	`memb_no`       INTEGER      NOT NULL COMMENT '회원번호', -- 회원번호
	`membername`    VARCHAR(50)  NOT NULL COMMENT '이름', -- 이름
	...
	`roadname_addr` VARCHAR(255) NOT NULL COMMENT '도로명주소', -- 도로명주소
	`base_addr`     VARCHAR(255) NOT NULL COMMENT '기본주소', -- 기본주소
	`addr`          VARCHAR(255) NOT NULL COMMENT '상세주소', -- 상세주소
	`memb_level`    VARCHAR(50)  NOT NULL COMMENT '회원구분', -- 회원구분
...
-- 메인카테고리
CREATE TABLE `BTSdb`.`gms_main_cate` (
...
-- 메인카테고리
ALTER TABLE `BTSdb`.`gms_main_cate`
...
-- 최근검색어
CREATE TABLE `BTSdb`.`gms_keyword` (
...
-- 활동지역
CREATE TABLE `BTSdb`.`gms_activelocal` (
	`act_local_no` INTEGER     NOT NULL COMMENT '활동지역번호', -- 활동지역번호
...
-- 활동지역
ALTER TABLE `BTSdb`.`gms_activelocal`
...
-- 모임목적
CREATE TABLE `BTSdb`.`gms_purpose` (
	`pups_no` INTEGER     NOT NULL COMMENT '모임목적번호', -- 모임목적번호
...
-- 모임태그
CREATE TABLE `BTSdb`.`gms_grouptag` (
...
-- 모임태그
ALTER TABLE `BTSdb`.`gms_grouptag`
...
-- 모임가입신청서
CREATE TABLE `BTSdb`.`gms_applyform` (
...
-- 모임가입신청서
ALTER TABLE `BTSdb`.`gms_applyform`
...
CREATE UNIQUE INDEX `UIX_gms_applyform`
	ON `BTSdb`.`gms_applyform` ( -- 모임가입신청서
	);
...
-- 모임내부 게시글 댓글
CREATE TABLE `BTSdb`.`gms_comment` (
...
-- 모임내부 일정
CREATE TABLE `BTSdb`.`gms_calendar` (
...
-- 모임내부 일정
ALTER TABLE `BTSdb`.`gms_calendar`
...
-- 모임맴버
CREATE TABLE `BTSdb`.`gms_join_memb` (
	`memb_no`       INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
	`g_no`          INTEGER NOT NULL COMMENT '모임번호', -- 모임번호
...
-- 모임맴버
ALTER TABLE `BTSdb`.`gms_join_memb`
...
-- 모금
CREATE TABLE `BTSdb`.`gms_accounting` (
...
-- 모금
ALTER TABLE `BTSdb`.`gms_accounting`
...
-- 맴버장소입력
CREATE TABLE `BTSdb`.`gms_select_midpoint` (
	...
	`memb_no`    INTEGER     NOT NULL COMMENT '회원번호', -- 회원번호
	`g_no`       INTEGER     NOT NULL COMMENT '모임번호', -- 모임번호
	`addr`       LONGTEXT    NOT NULL COMMENT '지번주소', -- 지번주소
	`cal_no`     INTEGER     NOT NULL COMMENT '일정번호' -- 일정번호
)
COMMENT '맴버장소입력';

-- 맴버장소입력
ALTER TABLE `BTSdb`.`gms_select_midpoint`
...
-- 고객센터 문의하기
CREATE TABLE `BTSdb`.`gms_notice_question` (
...
-- 고객센터 문의하기
ALTER TABLE `BTSdb`.`gms_notice_question`
...
CREATE INDEX `IX_gms_notice_question`
	ON `BTSdb`.`gms_notice_question`( -- 고객센터 문의하기
		`title` ASC -- 제목
	);
...
-- 관심활동지역
CREATE TABLE `BTSdb`.`gms_final_activelocal` (
	`memb_no`      INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
	`act_local_no` INTEGER NOT NULL COMMENT '활동지역번호' -- 활동지역번호
)
COMMENT '관심활동지역';
...
-- 관심활동지역
ALTER TABLE `BTSdb`.`gms_final_activelocal`
...
-- 관심모임목적
CREATE TABLE `BTSdb`.`gms_final_purpose` (
	`pups_no` INTEGER NOT NULL COMMENT '모임목적번호', -- 모임목적번호
	`memb_no` INTEGER NOT NULL COMMENT '회원번호' -- 회원번호
)
COMMENT '관심모임목적';
...
-- 관심모임목적
ALTER TABLE `BTSdb`.`gms_final_purpose`
...
-- 모금유형
CREATE TABLE `BTSdb`.`gms_accounting_cate` (
	`act_cate_no` INTEGER     NOT NULL COMMENT '모금유형번호', -- 모금유형번호
...
-- 모금유형
ALTER TABLE `BTSdb`.`gms_accounting_cate`
...
-- 모금납부내역
CREATE TABLE `BTSdb`.`gms_accounting_status` (
	`act_no`  INTEGER NOT NULL COMMENT '모금번호', -- 모금번호
	`g_no`    INTEGER NOT NULL COMMENT '모임번호', -- 모임번호
	`memb_no` INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
...
-- 모금납부내역
ALTER TABLE `BTSdb`.`gms_accounting_status`
...
-- 질문유형
CREATE TABLE `BTSdb`.`gms_query_cate` (
	`query_no` INTEGER     NOT NULL COMMENT '질문유형번호', -- 질문유형번호
...
-- 질문유형
ALTER TABLE `BTSdb`.`gms_query_cate`
...
-- SNS계정
CREATE TABLE `BTSdb`.`gms_final_SNS` (
	`memb_no`   INTEGER     NOT NULL COMMENT '회원번호', -- 회원번호
...
-- SNS계정
ALTER TABLE `BTSdb`.`gms_final_SNS`
...
-- 최종모임장소
CREATE TABLE `BTSdb`.`gms_destination` (
	...
	`reg_dt`      DATE        NOT NULL DEFAULT current_timestamp() COMMENT '등록일', -- 등록일
	`view_cnt`    INTEGER     NOT NULL DEFAULT 1 COMMENT '선택횟수' -- 선택횟수
)
COMMENT '최종모임장소';

-- 최종모임장소
ALTER TABLE `BTSdb`.`gms_destination`
...
-- 최종모임장소 유니크 인덱스
CREATE UNIQUE INDEX `UIX_gms_destination`
	ON `BTSdb`.`gms_destination` ( -- 최종모임장소
		`location_id` ASC -- 장소아이디
	);
...
-- 모임등급유형
CREATE TABLE `BTSdb`.`gms_memb_grade` (
	`g_memb_grd_no` INTEGER     NOT NULL COMMENT '모임등급유형번호', -- 모임등급유형번호
...
-- 모임등급유형
ALTER TABLE `BTSdb`.`gms_memb_grade`
...
-- 신고유형
CREATE TABLE `BTSdb`.`gms_report_cate` (
	`rpt_cate_no` INTEGER     NOT NULL COMMENT '신고유형번호', -- 신고유형번호
...
-- 신고유형
ALTER TABLE `BTSdb`.`gms_report_cate`
...
-- 초대장저장공간
CREATE TABLE `BTSdb`.`gms_invite_box` (
...
-- 초대장저장공간
ALTER TABLE `BTSdb`.`gms_invite_box`
...
-- 모임가입신청서질문목록
CREATE TABLE `BTSdb`.`gms_applyform_question` (
...
-- 모임가입신청서질문목록
ALTER TABLE `BTSdb`.`gms_applyform_question`
...
-- 모임가입신청서질문목록답변
CREATE TABLE `BTSdb`.`gms_applyform_answer` (
	`memb_no`          INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
	`appl_question_no` INTEGER  NOT NULL COMMENT '모임가입신청서질문목록번호', -- 모임가입신청서질문목록번호
...
-- 모임가입신청서질문목록답변
ALTER TABLE `BTSdb`.`gms_applyform_answer`
...
-- 가입신청서기본답변
CREATE TABLE `BTSdb`.`gms_applyform_default` (
	`g_no`    INTEGER  NOT NULL COMMENT '모임번호', -- 모임번호
	`memb_no` INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
...
-- 가입신청서기본답변
ALTER TABLE `BTSdb`.`gms_applyform_default`
	ADD CONSTRAINT `PK_gms_applyform_default` -- 가입신청서기본답변 기본키
		PRIMARY KEY (
			`g_no`,    -- 모임번호
			`memb_no`  -- 회원번호
		);
...
ALTER TABLE `BTSdb`.`gms_group`
	ADD CONSTRAINT `FK_gms_activelocal_TO_gms_group` -- 활동지역 -> 소모임
		...
		REFERENCES `BTSdb`.`gms_activelocal` ( -- 활동지역
			`act_local_no` -- 활동지역번호
		);
...
ALTER TABLE `BTSdb`.`gms_group`
	ADD CONSTRAINT `FK_gms_purpose_TO_gms_group` -- 모임목적 -> 소모임
		...
		REFERENCES `BTSdb`.`gms_purpose` ( -- 모임목적
			`pups_no` -- 모임목적번호
		);
...
ALTER TABLE `BTSdb`.`gms_grouptag`
	ADD CONSTRAINT `FK_gms_group_TO_gms_grouptag` -- 소모임 -> 모임태그
		...
		REFERENCES `BTSdb`.`gms_group` ( -- 소모임
			`g_no` -- 모임번호
		);
...
ALTER TABLE `BTSdb`.`gms_board`
	ADD CONSTRAINT `FK_gms_join_memb_TO_gms_board` -- 모임맴버 -> 모임게시글
		...
		REFERENCES `BTSdb`.`gms_join_memb` ( -- 모임맴버
			`memb_no`, -- 회원번호
			`g_no`     -- 모임번호
		);
...
ALTER TABLE `BTSdb`.`gms_comment`
	ADD CONSTRAINT `FK_gms_board_TO_gms_comment` -- 모임게시글 -> 모임내부 게시글 댓글
		...
		REFERENCES `BTSdb`.`gms_board` ( -- 모임게시글
			`b_no` -- 게시글번호
		);
...
ALTER TABLE `BTSdb`.`gms_join_memb`
	ADD CONSTRAINT `FK_gms_memb_grade_TO_gms_join_memb` -- 모임등급유형 -> 모임맴버
		...
		REFERENCES `BTSdb`.`gms_memb_grade` ( -- 모임등급유형
			`g_memb_grd_no` -- 모임등급유형번호
		);
...
ALTER TABLE `BTSdb`.`gms_accounting`
	ADD CONSTRAINT `FK_gms_accounting_cate_TO_gms_accounting` -- 모금유형 -> 모금
		...
		REFERENCES `BTSdb`.`gms_accounting_cate` ( -- 모금유형
			`act_cate_no` -- 모금유형번호
		);
...
-- 맴버장소입력
ALTER TABLE `BTSdb`.`gms_select_midpoint`
	ADD CONSTRAINT `FK_gms_calendar_TO_gms_select_midpoint` -- 모임내부 일정 -> 맴버장소입력
		FOREIGN KEY (
			`cal_no` -- 일정번호
		)
		REFERENCES `BTSdb`.`gms_calendar` ( -- 모임내부 일정
			`cal_no` -- 일정번호
		);
...
ALTER TABLE `BTSdb`.`gms_faq`
	ADD CONSTRAINT `FK_gms_main_cate_TO_gms_faq` -- 메인카테고리 -> 고객센터 자주하는 질문
		...
		REFERENCES `BTSdb`.`gms_main_cate` ( -- 메인카테고리
			`main_cate_no` -- 메인카테고리번호
		);
...
ALTER TABLE `BTSdb`.`gms_faq`
	ADD CONSTRAINT `FK_gms_query_cate_TO_gms_faq` -- 질문유형 -> 고객센터 자주하는 질문
		...
		REFERENCES `BTSdb`.`gms_query_cate` ( -- 질문유형
			`query_no` -- 질문유형번호
		);
...
ALTER TABLE `BTSdb`.`gms_accounting_status`
	ADD CONSTRAINT `FK_gms_accounting_TO_gms_accounting_status` -- 모금 -> 모금납부내역
		...
		REFERENCES `BTSdb`.`gms_accounting` ( -- 모금
			`act_no` -- 모금번호
		);
...
ALTER TABLE `BTSdb`.`gms_report`
	ADD CONSTRAINT `FK_gms_report_cate_TO_gms_report` -- 신고유형 -> 신고
		...
		REFERENCES `BTSdb`.`gms_report_cate` ( -- 신고유형
			`rpt_cate_no` -- 신고유형번호
		);
...
-- 모임가입신청서질문목록
ALTER TABLE `BTSdb`.`gms_applyform_question`
	ADD CONSTRAINT `FK_gms_group_TO_gms_applyform_question` -- 소모임 -> 모임가입신청서질문목록
		FOREIGN KEY (
			`g_no` -- 모임번호
		)
		REFERENCES `BTSdb`.`gms_group` ( -- 소모임
...
-- 모임가입신청서질문목록답변
ALTER TABLE `BTSdb`.`gms_applyform_answer`
	ADD CONSTRAINT `FK_gms_memb_TO_gms_applyform_answer` -- 회원 -> 모임가입신청서질문목록답변
		FOREIGN KEY (
			`memb_no` -- 회원번호
		)
		REFERENCES `BTSdb`.`gms_memb` ( -- 회원
...
ALTER TABLE `BTSdb`.`gms_applyform_answer`
	ADD CONSTRAINT `FK_gms_applyform_question_TO_gms_applyform_answer` -- 모임가입신청서질문목록 -> 모임가입신청서질문목록답변
		...
		REFERENCES `BTSdb`.`gms_applyform_question` ( -- 모임가입신청서질문목록
			`appl_question_no` -- 모임가입신청서질문목록번호
		);

-- 가입신청서기본답변
ALTER TABLE `BTSdb`.`gms_applyform_default`
	ADD CONSTRAINT `FK_gms_group_TO_gms_applyform_default` -- 소모임 -> 가입신청서기본답변
		FOREIGN KEY (
			`g_no` -- 모임번호
		)
		REFERENCES `BTSdb`.`gms_group` ( -- 소모임
...
-- 가입신청서기본답변
ALTER TABLE `BTSdb`.`gms_applyform_default`
	ADD CONSTRAINT `FK_gms_memb_TO_gms_applyform_default` -- 회원 -> 가입신청서기본답변
		FOREIGN KEY (
			`memb_no` -- 회원번호
		)
		REFERENCES `BTSdb`.`gms_memb` ( -- 회원
