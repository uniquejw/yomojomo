...
CREATE TABLE gms_group (
  ...
  cdate      DATE         NOT NULL DEFAULT current_timestamp() COMMENT '모임생성일', -- 모임생성일
  logo       VARCHAR(255) NULL     COMMENT '모임로고사진', -- 모임로고사진
  intro      LONGTEXT     NOT NULL COMMENT '모임안내문구', -- 모임안내문구
  mcnt       INTEGER      NOT NULL DEFAULT 1 COMMENT '모임인원수', -- 모임인원수
  fee        INTEGER      NOT NULL DEFAULT 0 COMMENT '회비', -- 회비
  vcnt       INTEGER      NOT NULL DEFAULT 0 COMMENT '방문자수' -- 방문자수
)
COMMENT '소모임';
...
CREATE TABLE gms_applyform (
  ...
  appldt  DATE     NOT NULL DEFAULT current_timestamp() COMMENT '신청일' -- 신청일
)
COMMENT '모임가입신청서';
...
CREATE TABLE gms_board (
  ...
  gno     INTEGER  NOT NULL COMMENT '모임번호', -- 모임번호
  membno  INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
  dt      DATE     NOT NULL DEFAULT current_timestamp() COMMENT '작성일시', -- 작성일시
  content LONGTEXT NOT NULL COMMENT '내용', -- 내용
  viewcnt INTEGER  NOT NULL DEFAULT 0 COMMENT '조회수' -- 조회수
)
COMMENT '모임게시글';
...
CREATE TABLE gms_comment (
  ...
  membno  INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
  gno     INTEGER  NOT NULL COMMENT '모임번호', -- 모임번호
  regdt   DATE     NOT NULL DEFAULT current_timestamp() COMMENT '등록일' -- 등록일
)
COMMENT '모임내부 게시글 댓글';
...
CREATE TABLE gms_accounting (
  ...
  dt        DATE         NOT NULL DEFAULT current_timestamp() COMMENT '등록일', -- 등록일
...
CREATE TABLE gms_faq (
  ...
  appdt    DATE         NOT NULL DEFAULT current_timestamp() COMMENT '등록일' -- 등록일
)
...
CREATE TABLE gms_notice_question (
  ...
  querydt  DATE         NOT NULL DEFAULT current_timestamp() COMMENT '문의일', -- 문의일
  answer   LONGTEXT     NULL     COMMENT '답변', -- 답변
  answerdt DATE         NULL     DEFAULT current_timestamp() COMMENT '답변일' -- 답변일
)
COMMENT '고객센터 문의하기';
...
CREATE TABLE gms_destination (
  ...
  regdt       DATE        NOT NULL DEFAULT current_timestamp() COMMENT '등록일', -- 등록일
  cnt         INTEGER     NOT NULL DEFAULT 0 COMMENT '선택횟수' -- 선택횟수
)
COMMENT '최종모임장소';
...
CREATE TABLE gms_recruit (
  ...
  content    LONGTEXT    NOT NULL COMMENT '내용', -- 내용
  reddt      DATE        NOT NULL DEFAULT current_timestamp() COMMENT '작성일', -- 작성일
  viewcnt    INTEGER     NOT NULL DEFAULT 0 COMMENT '조회수', -- 조회수
  itrlocalno INTEGER     NOT NULL COMMENT '활동지역번호', -- 활동지역번호
  fileno     INTEGER     NOT NULL COMMENT '모임목적번호', -- 모임목적번호
  membno     INTEGER     NOT NULL COMMENT '회원번호' -- 회원번호
