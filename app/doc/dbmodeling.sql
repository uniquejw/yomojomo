...
-- 메인카테고리2
DROP TABLE IF EXISTS `BTSdb`.`gms_mcate2` RESTRICT;
...
-- 직군
DROP TABLE IF EXISTS `BTSdb`.`gms_itr_jobgroup` RESTRICT;

-- 직군세부
DROP TABLE IF EXISTS `BTSdb`.`gms_itr_jobgroup_detail` RESTRICT;
...
-- 관심직군
DROP TABLE IF EXISTS `BTSdb`.`gms_final_job` RESTRICT;
...
-- 리쿠르팅게시판
DROP TABLE IF EXISTS `BTSdb`.`gms_recruit` RESTRICT;
...
-- 직군
CREATE TABLE `BTSdb`.`gms_itr_jobgroup` (
...
-- 직군
ALTER TABLE `BTSdb`.`gms_itr_jobgroup`
...
-- 직군세부
CREATE TABLE `BTSdb`.`gms_itr_jobgroup_detail` (
...
-- 직군세부
ALTER TABLE `BTSdb`.`gms_itr_jobgroup_detail`
...
CREATE UNIQUE INDEX `UIX_gms_itr_jobgroup_detail`
  ON `BTSdb`.`gms_itr_jobgroup_detail` ( -- 직군세부
    `name` ASC -- 관심직군세부명
  );
...
CREATE TABLE `BTSdb`.`gms_comment` (
  ...
  `bno`    INTEGER  NOT NULL COMMENT '게시글번호', -- 게시글번호
  `conent` LONGTEXT NOT NULL COMMENT '댓글내용 ', -- 댓글내용 
  `membno` INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
  `gno`    INTEGER  NOT NULL COMMENT '모임번호', -- 모임번호
...
CREATE TABLE `BTSdb`.`gms_notice_question` (
  ...
  `membno`   INTEGER      NOT NULL COMMENT '회원번호', -- 회원번호
  `title`    VARCHAR(255) NOT NULL COMMENT '제목', -- 제목
  `name`     LONGTEXT     NOT NULL COMMENT '내용', -- 내용
...
-- 관심직군
CREATE TABLE `BTSdb`.`gms_final_job` (
  `membno` INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
  `ijgno`  INTEGER NOT NULL COMMENT '직군번호' -- 직군번호
)
COMMENT '관심직군';
...
-- 관심직군
ALTER TABLE `BTSdb`.`gms_final_job`
...
-- 리쿠르팅게시판
CREATE TABLE `BTSdb`.`gms_recruit` (
...
ALTER TABLE `BTSdb`.`gms_itr_jobgroup_detail`
  ADD CONSTRAINT `FK_gms_itr_jobgroup_TO_gms_itr_jobgroup_detail` -- 직군 -> 직군세부
    ...
    REFERENCES `BTSdb`.`gms_itr_jobgroup` ( -- 직군
      `ijgno` -- 직군번호
    );
...
ALTER TABLE `BTSdb`.`gms_faq`
  ADD CONSTRAINT `FK_gms_mcate2_TO_gms_faq` -- 메인카테고리2 -> 고객센터 자주하는 질문
    ...
    REFERENCES `BTSdb`.`gms_mcate2` ( -- 메인카테고리2
      `mcate2no` -- 메인카테고리2번호
    );
